<template>
  <n-card embedded :bordered="false">
        ...
        <template #header-extra>
          <n-space justify="end">
            ...
            <n-button
              strong
              secondary
              @click="message.success('您应该不需要，需要的话加一个弹框')"
              type="error"
            >
              权限详情
            </n-button>
    ...
    <n-modal v-model:show="editShowModal" :key="editShowModal">
      <n-card
        :style="{
          width: clientWidth > 1025 ? (tabKey != '1' ? '500px' : '60%') : '96%',
        }"
        title="正在编辑信息"
        :bordered="false"
        size="huge"
        ...
        <n-spin :show="editUserInfoLoading">
          <n-tabs type="segment" v-model:value="tabKey">
                ...
                <n-grid cols="2" x-gap="10" item-responsive>
                  <n-grid-item span="2 820:1 1025:1">
                    <n-form-item label="年龄">
                      <n-input-number
                        style="width: 100%"
                        v-model:value="editModel.userAge"
                        clearable
                        placeholder="请输入年龄"
                      />
                    </n-form-item>
                  ...
                  <n-grid-item span="2 820:1 1025:1">
                    <n-form-item label="性别">
                      <n-select
                        v-model:value="editModel.userSex"
                        placeholder="请选择性别"
                        :options="sexOptions"
                      />
                    </n-form-item>
            ...
            <n-tab-pane name="2" tab="修改头像">
              <br />
              <n-space justify="center">
                <n-avatar :size="100" :src="updateHeader" />
              </n-space>
              <n-divider />
              <n-space justify="center">
                <!--        图片上传，请修改成自己接口-->
              ...
              <n-form>
                <n-form-item-row label="新密码">
                  <n-input
                    ...
                    placeholder="请输入密码"
                    v-model:value="updatePwdForm.userPassword"
                    type="password"
                  />
                </n-form-item-row>
                <n-form-item-row label="确认新密码">
                  <n-input
                    ...
                    show-password-on="mousedown"
                    placeholder="请输入确认密码"
                    v-model:value="updatePwdForm.newUserPassword"
                    type="password"
                  />
                </n-form-item-row>
                <n-divider />
            ...
            <n-tab-pane name="4" tab="修改用户名">
              <br />
              <n-input
                ref="updateNewNameRef"
                v-model:value="userNewName"
                placeholder="请输入新用户名"
              />
              <n-divider />
              <n-space justify="center">
...
import { InputInst, NIcon, useMessage } from "naive-ui";
...
const tabKey = ref("1");
updateHeader.value = userInfo.value.avatar!;
...
const onBeforeUpload = (file: any, fileList: any) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    imageConversion.compressAccurately(file.file.file, 20).then((res) => {
      file.file.file = new File([res], file.file.file.name, {
        type: res.type,
